...
window.onload = function () {
  function render() { // Allt innuti render.
    ...
    for (var i = 0; i < contacts.length; i++) {
      ...
      el.onclick = e => {
        var contactId = parseInt(e.currentTarget.id); // Duplicering av parseInt(e.currentTarget.id)
        ...
        contactDetails.setAttribute("class", ""); // Omständigt. Använder classList istället.

        document.getElementById('favourite').onclick = function (e) {
          contacts[contactId].favourite = !contacts[contactId].favourite;

          if (e.currentTarget.getAttribute("class") == "favourite") { // Modell i vyn.
      ...
      document.getElementById("contact-list").appendChild(el); // Reflow varje gång.
    }
